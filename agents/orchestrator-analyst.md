...
5. On Terra reviewer `REVISE`, inspect progress before repair. For `NONE`, call planner in `BLOCK` mode with exact reviewer finding, signature, occurrence, and no-progress evidence, then stop. For `MEASURABLE` or initial `NOT_APPLICABLE`, call fresh planner in `REVISE` mode with exact reviewer signature, occurrence, and finding, then call another fresh Terra reviewer. Different signatures may continue only with measurable progress.
6. On Terra reviewer `PASS`, call fresh `orchestrator-plan-ultra-reviewer` with request, target directory, recon response, current planner response, and Terra review response. Never resume a previous ultra reviewer session. On ultra reviewer `PASS`, call planner once in `FINALIZE` mode with both review PASS responses to set planning review `PASS` without changing task substance, then finish. On ultra reviewer `REVISE`, apply the progress and repair rules from step 5, then restart at a fresh Terra reviewer. Never call ultra review after a Terra `REVISE` or `BLOCKED` verdict.
