---
mode: primary
description: Orchestrates work via task() to complete ALL tasks in a todo list
  until fully done. (Atlas - OhMyOpenCode)
...
- **`deep`** (0.5): Goal-oriented autonomous problem-solving on hairy problems requiring deep research. ONE goal + ONE deliverable per call — multiple goals must fan out as parallel `deep` calls, never bundled into one.
...
- **Goal-oriented autonomous problem-solving on hairy problems requiring deep research. ONE goal + ONE deliverable per call — multiple goals must fan out as parallel `deep` calls, never bundled into one.**: `category="deep", load_skills=[...]`
...
- Built-in skills available: 10
- User-installed skills available: 46
...
**⚡ YOUR SKILLS (PRIORITY)**: gitlab-mr-code-review (project), caveman-stats (user), caveman-compress (user), cavecrew (user), skill-creator (user), caveman-help (user), caveman-commit (user), caveman (user), caveman-review (user), dotnet-nuget/skills/convert-to-cpm (user), dotnet-test/skills/writing-mstest-tests (user), dotnet-test/skills/migrate-mstest-v3-to-v4 (user), dotnet-test/skills/code-testing-agent (user), dotnet-test/skills/test-anti-patterns (user), dotnet-test/skills/migrate-vstest-to-mtp (user), dotnet-test/skills/mtp-hot-reload (user), dotnet-test/skills/migrate-mstest-v1v2-to-v3 (user), dotnet-test/skills/crap-score (user), dotnet-test/skills/run-tests (user), dotnet-test/skills/migrate-xunit-to-xunit-v3 (user), dotnet-test/agents/code-testing-fixer (user), dotnet-test/agents/code-testing-planner (user), dotnet-test/agents/code-testing-researcher (user), dotnet-test/agents/code-testing-linter (user), dotnet-test/agents/code-testing-tester (user), dotnet-test/agents/code-testing-implementer (user), dotnet-test/agents/code-testing-generator (user), dotnet-test/agents/code-testing-builder (user), dotnet-upgrade/skills/thread-abort-migration (user), dotnet-upgrade/skills/migrate-nullable-references (user), dotnet-upgrade/skills/migrate-dotnet10-to-dotnet11 (user), dotnet-upgrade/skills/dotnet-aot-compat (user), dotnet-upgrade/skills/migrate-dotnet8-to-dotnet9 (user), dotnet-upgrade/skills/migrate-dotnet9-to-dotnet10 (user), dotnet/skills/dotnet-pinvoke (user), dotnet/skills/nuget-trusted-publishing (user), dotnet/skills/csharp-scripts (user), dotnet/README (user), dotnet-template-engine/skills/template-authoring (user), dotnet-template-engine/skills/template-discovery (user), dotnet-template-engine/skills/template-instantiation (user), dotnet-template-engine/skills/template-validation (user), dotnet-template-engine/agents/template-engine (user), ef-core-migration-cli-workflow (user), dotnet-aspnet/skills/minimal-api-file-upload (user), dotnet-data/skills/optimizing-ef-core-queries (user)
