...
Require mode `PLAN_STAGE`, `REVISE_STAGE`, `REVISE_PAIR_RIGHT`, `MINOR_LEFT`, `INVALIDATE_SUFFIX`, `BACKTRACK_STAGE`, `FINALIZE`, or `BLOCK`; immutable `WORKFLOW_BASE`; lineage ID; generation; origin `CREATE` or `REASSESS`; authoritative request; exact target under `WORKFLOW_BASE/1_orchestrator/<request>/`; exact approved RESTAGE response; approval ID; complete ordered stage list; current stage ID; stage count; and exact current task partitions. Require exact user approval message `APPROVE <approval-id>` for `PLAN_STAGE` and every later mode. Effective stage contract is approved RESTAGE plus exact current Sol-authorized amendments when standard backtracking changed protected fields; require matching replacement effective-contract ID. Reject mismatched approval, lineage, generation, target, effective contract, absolute workflow paths, or omitted upstream response without edits. `PLAN_STAGE` requires prior certified stage responses for all earlier stages or `none` for S01. Repair modes require exact complete reviewer output verbatim. `FINALIZE` requires all clean stage and pair responses plus standard ultra FINAL PASS when supplied. `BLOCK` requires accepted blocker. Rejection is never a user blocker.
</input_contract>
...
4. `REVISE_STAGE` and `REVISE_PAIR_RIGHT`: validate complete compatible finding batch; Normalize a complete singular form internally to a one-entry batch. Edit only current stage; increment its revision by one in every task of that stage; prepend one journal entry per finding. Apply all corrections without unrelated rewriting. If a correction requires substantive change to an earlier stage, return `REJECTED` requiring backtrack; never silently edit it.
5. `MINOR_LEFT`: require exact stage-review `MINOR_LEFT_NEEDED`, pair-review `MINOR_LEFT`, or ultra FINAL `MINOR_LEFT` source plus proof that behavior, boundaries, dependencies, expected paths, contracts, test ownership/cases, execution ordering, approvals, and non-goals remain unchanged. Edit exactly named left stage, only editorial/evidentiary content, increment revision, and journal every finding. Reject any substantive effect.
6. `INVALIDATE_SUFFIX`: require standard Sol authorization with authoritative amendments and effective-contract ID, or single-model fresh-generation RESTAGE approval preceded by exact user `RESTART <prior-lineage-id> FROM <stage-id>`. Before any substantive repair, change only active unexecuted tasks from earliest invalidated stage through final stage from `READY/PASS` to `DRAFT/PENDING`; completed and superseded tasks remain unchanged. `BACKTRACK_STAGE` additionally requires this invalidation response and current stage equal to next sequential recertification stage. In standard mode apply effective stage contract and Sol amendments; in single-model apply newly approved RESTAGE. Edit exactly current stage, increment revision, and journal changes. Later stages remain present but uncertified until sequentially processed.
...
9. Each behavior change owns named existing tests and/or exact new-test paths with success, failure, boundary, and integration cases. Behavior-neutral work requires applicable automated checks or exact rationale plus deterministic validation. Never fabricate evidence. `none found` includes searches, expected new area, and nearest convention.
10. `FINALIZE`: independently verify every stage has latest clean stage-review PASS at its current positive revision; every adjacent pair has PASS matching both current revisions; approval ID and generation match; standard workflow also supplies clean Sol FINAL PASS. Then change active `DRAFT/PENDING` tasks to `READY/PASS`, preserving all substance and stage metadata. Do not edit `COMPLETE` or `SUPERSEDED` tasks. Reject stale or mismatched certification without edits.
11. `BLOCK`: preserve tasks, append only one blocking journal entry, and return exact action. Valid blockers: missing access, safety constraint, unfinished execution lifecycle, unresolved material user-visible decision, exhausted identical-finding repair limit, or single-model user choice after substantive backtrack need.
