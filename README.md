...
Planner возвращает `REJECTED` без edits для malformed/contradictory mode input, несовместимого finding batch или target collision. Это не user blocker: analyst повторяет bounded malformed call, отправляет rejected revision в fresh exhaustive review, перезапускает required review chain после rejected finalization либо после collision пробует deterministic suffixes `-2`, `-3`, ... без чтения workflow directory. Только planner проверяет существование exact candidate target через exact-path `read`, поэтому ignored existing targets и races не пропускаются. Immediate blocker остаётся отдельным от findings: `Findings: none` плюс exact blocker, user action и доказательство невозможности bounded correction.
