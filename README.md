...
`REVISE` и другие corrective statuses — внутренние переходы controller, не blockers. Analyst немедленно передаёт полный approved contract, exact planner output и exact reviewer finding обратно planner, затем запускает fresh review новой revision. `REJECTED`, malformed или stale payload повторяется той же ролью и mode с полным authoritative payload и exact rejection evidence. Просьба подагента «повторить», «перепланировать» или «перезапустить» не показывается пользователю и не завершает workflow, если contract status nonterminal или `Блокер: none`.
...
python3 tests/test-analyst-replanning-e2e.py
```

Три последние команды обязательны после любых изменений и требуют установленный и авторизованный OpenCode. Первая поднимает isolated `opencode serve --pure`, создаёт временный workspace fixture, проводит analyst через no-question terminal PASS и approval, проверяет S01/S02 `PASS` на revision 1 без `REVISE`, pair review, `FINALIZE`, отсутствие executor calls, synthetic user turns и product writes. Вторая отвечает первому native question batch через session-scoped V2 question API с legacy fallback для legacy sessions, требует fresh DISCOVERY, затем второй batch из двух вопросов, ещё один DISCOVERY, terminal `PASS_NO_QUESTIONS`, единственный accepted RESTAGE из трёх этапов, обе adjacent pair проверки, три READY task и отсутствие обычных user messages для ответов. Третья требует stage `REVISE`, planner `REVISE_STAGE`, fresh revision PASS и `FINALIZE` в том же runner turn; она проверяет verbatim approved RESTAGE/planner/reviewer payloads в repair handoff и запрещает user-facing restart/replan blocker. Tests запрещают question/discovery после RESTAGE. Допускаются максимум три bounded malformed `REJECTED` либо contract-invalid `BLOCKED` retry и максимум три consecutive identity-equivalent accepted replays без изменения routing state; conflicting или nonconsecutive replay падает. Substantive `BLOCKED` всегда завершает acceptance failure. Initial prompts отправляются через async API. Harness использует официальный V2 session wait, а при документированном `503 Session wait is not available yet` для legacy session API переключается на bounded legacy status polling. Isolation использует `OPENCODE_TEST_HOME`, отключение project config, autoupdate, autocompact и models fetch. Fatal provider/child-task errors завершают test сразу; отсутствие новых messages/tool states завершается через 300 секунд. Каждый run печатает `ANALYST_E2E_TIMING` с checkpoints, длительностью task calls и token usage. Общий timeout по умолчанию 1800 секунд. Live tests используют `openai/gpt-5.6-luna`; переопределения: `ANALYST_E2E_MODEL`, `ANALYST_E2E_TIMEOUT_SECONDS` и `ANALYST_E2E_NO_PROGRESS_SECONDS`.
