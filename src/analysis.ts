...
function mapById<T extends { id: string }>(items: T[], field: string): Map<string, T> {
  ...
  for (const item of items) {
    if (result.has(item.id)) throw new ProtocolError(field, "duplicate identifier", item.id)
    result.set(item.id, item)
  }
  return result
}

function ensureContiguous<T extends { id: string }>(items: T[], family: string, field: string): void {
  items.forEach((item, index) => {
    const expected = `${family}-${String(index + 1).padStart(3, "0")}`
    if (item.id !== expected) throw new ProtocolError(`${field}[${index}].id`, `${family} identifiers must be contiguous and ordered`, item.id)
  })
}

export function requiredNfrCategories(surfaces: string[]): Set<string> {
  const result = new Set<string>()
  for (const surface of surfaces) for (const category of SURFACE_NFR[surface] ?? []) result.add(category)
  return result
}

export function hasDependency(stages: Map<string, AnalysisStage>, consumer: string, producer: string): boolean {
...
export function validateAnalysis(input: unknown): Analysis {
  ...
  if (root.schema_version !== ANALYSIS_SCHEMA_VERSION) throw new ProtocolError("schema_version", `must be ${ANALYSIS_SCHEMA_VERSION}`, root.schema_version)
  ...
  for (const surface of surfaces) if (!CHANGE_SURFACES.has(surface)) throw new ProtocolError("change_surfaces", "unsupported value", surface)
  ...
  const stages = array(root.stages, "stages").map((raw, index) => {
    ...
    if (id !== `S${String(index + 1).padStart(2, "0")}`) throw new ProtocolError(`stages[${index}].id`, "stages must be contiguous and ordered", id)
    const slug = text(item.slug, `stages[${index}].slug`)
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) throw new ProtocolError(`stages[${index}].slug`, "must be lower kebab-case", slug)
    ...
    for (const dependency of depends) if (!earlier.has(dependency)) throw new ProtocolError(`stages[${index}].depends_on`, "must reference earlier stages", dependency)
    return {
      id,
  ...
  if (!stages.length) throw new ProtocolError("stages", "must not be empty")
  ...
  const nfrs = array(root.nfrs, "nfrs").map((raw, index) => {
    ...
    if (!NFR_CATEGORIES.has(category)) throw new ProtocolError(`nfrs[${index}].category`, "unsupported category", category)
  ...
  for (const item of [...requirements, ...nfrs]) {
    ...
    if (!owner) throw new ProtocolError(`${item.id}.stage`, "unknown stage", item.stage)
    const ownerList = item.id.startsWith("REQ-") ? owner.requirements : owner.nfrs
    if (!ownerList.includes(item.id)) throw new ProtocolError(item.id, "owning stage does not list item", item.stage)
    for (const ac of item.acceptance) {
      const target = acMap.get(ac)
      if (!target || target.stage !== item.stage) throw new ProtocolError(`${item.id}.acceptance`, "acceptance must exist in owning stage", ac)
    }
    for (const scenario of item.scenarios) {
      const target = scenarioMap.get(scenario)
      if (!target || target.stage !== item.stage || !target.requirements.includes(item.id)) throw new ProtocolError(`${item.id}.scenarios`, "scenario link must be reciprocal in owning stage", scenario)
    }
  }
  const referencedAcceptance = new Set<string>()
  for (const item of [...requirements, ...nfrs]) for (const id of item.acceptance) referencedAcceptance.add(id)
  for (const criterion of acceptance) {
    if (!stageMap.has(criterion.stage)) throw new ProtocolError(`${criterion.id}.stage`, "unknown stage", criterion.stage)
    if (!referencedAcceptance.has(criterion.id)) throw new ProtocolError(criterion.id, "acceptance criterion is not linked from any requirement")
  }
  for (const scenario of scenarios) {
    if (!stageMap.has(scenario.stage)) throw new ProtocolError(`${scenario.id}.stage`, "unknown stage", scenario.stage)
    for (const linked of scenario.requirements) {
      const requirement = requirementMap.get(linked) ?? nfrMap.get(linked)
      if (!requirement) throw new ProtocolError(`${scenario.id}.requirements`, "unknown requirement", linked)
  ...
  for (const contract of contracts) {
    if (contract.external && contract.producer !== null) throw new ProtocolError(contract.id, "external contract cannot have an internal producer")
    if (!contract.external && contract.producer === null) throw new ProtocolError(contract.id, "internal contract requires a producer")
    if (!contract.consumers.length && !contract.terminal) throw new ProtocolError(contract.id, "contract without consumers must be terminal")
    if (contract.producer) {
      const producer = stageMap.get(contract.producer)
      if (!producer?.contracts_produced.includes(contract.id)) throw new ProtocolError(contract.id, "producer stage does not list contract")
    }
    for (const consumerId of contract.consumers) {
      const consumer = stageMap.get(consumerId)
      if (!consumer?.contracts_consumed.includes(contract.id)) throw new ProtocolError(contract.id, "consumer stage does not list contract", consumerId)
      if (contract.producer && !hasDependency(stageMap, consumerId, contract.producer)) throw new ProtocolError(contract.id, "consumer dependency graph omits producer", { producer: contract.producer, consumer: consumerId })
    }
  }
  for (const stage of stages) {
    for (const id of stage.requirements) {
      const item = requirementMap.get(id)
      if (!item) throw new ProtocolError(stage.id, "unknown requirement", id)
      if (item.stage !== stage.id) throw new ProtocolError(stage.id, "stage lists a requirement owned by another stage", id)
    }
    for (const id of stage.nfrs) {
      const item = nfrMap.get(id)
      if (!item) throw new ProtocolError(stage.id, "unknown NFR", id)
      if (item.stage !== stage.id) throw new ProtocolError(stage.id, "stage lists an NFR owned by another stage", id)
    }
    for (const id of [...stage.contracts_consumed, ...stage.contracts_produced]) if (!contractMap.has(id)) throw new ProtocolError(stage.id, "unknown contract", id)
  }
  ...
  const applicability = array(root.nfr_applicability, "nfr_applicability").map((raw, index) => {
    ...
    if (!NFR_CATEGORIES.has(category)) throw new ProtocolError(`nfr_applicability[${index}].category`, "unsupported category", category)
    const status = text(item.status, `nfr_applicability[${index}].status`)
    if (!new Set(["required", "not_applicable", "deferred"]).has(status)) throw new ProtocolError(`nfr_applicability[${index}].status`, "unsupported status", status)
    ...
    if (status === "required" && (!owner || !acceptanceIds.length)) throw new ProtocolError(`nfr_applicability[${index}]`, "required category needs owner and acceptance")
    if (status !== "required" && acceptanceIds.length) throw new ProtocolError(`nfr_applicability[${index}]`, "non-required category cannot claim acceptance")
    if (owner && !stageMap.has(owner)) throw new ProtocolError(`nfr_applicability[${index}].owner`, "unknown stage", owner)
    for (const id of acceptanceIds) {
      const criterion = acMap.get(id)
      if (!criterion) throw new ProtocolError(`nfr_applicability[${index}].acceptance`, "unknown acceptance", id)
      if (owner && criterion.stage !== owner) throw new ProtocolError(`nfr_applicability[${index}].acceptance`, "acceptance must belong to owner stage", id)
    }
    return { category, status: status as "required" | "not_applicable" | "deferred", evidence: text(item.evidence, `nfr_applicability[${index}].evidence`), owner, acceptance: acceptanceIds }
  })
  const categories = new Set(applicability.map((item) => item.category))
  for (const required of requiredNfrCategories(surfaces)) if (!categories.has(required)) throw new ProtocolError("nfr_applicability", "missing category implied by change surfaces", required)
...
export function affectedStageClosure(analysisInput: unknown, seedsInput: string[]): string[] {
  ...
  for (const seed of seeds) if (!stages.has(seed)) throw new ProtocolError("affected_stages", "unknown seed", seed)
