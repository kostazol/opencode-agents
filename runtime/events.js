import { AGENT_ACTIONS, EVENT_BY_ACTION, ProtocolError, affectedStageClosure, clone, exactFields, record, strings, text, validateAnalysis } from "./orchestrator.js";
import { normalizeProgress, sha, stageMap, stagesFromAnalysis, validateState } from "./state.js";
import { applyReopen, block, choice, feedbackText, proposeReopen, recordRevise, requireRevision } from "./review.js";
async function applyStage(base, state, pending, payload, analysis) {
    const stage = stageMap(state).get(pending.stage ?? "");
    if (!stage)
        throw new ProtocolError("state.pending.stage", "stage action requires known stage", pending.stage);
    ...
    if (pending.action === "REVIEW_STAGE") {
        ...
        if (status === "REVISE") {
            ...
            if (result.stalled)
                return block(state, pending, "no_semantic_progress", `Repeated unchanged findings: ${result.summary}`);
            return { status: state.status, stage: stage.id, next_revision: stage.revision + 1 };
        }
        if (status === "REOPEN") {
            if (!analysis)
                throw new ProtocolError("analysis", "reopening requires analysis.json");
...
async function applyNonStage(base, state, pending, payload, analysis) {
    if (pending.action === "DISCOVER") {
        ...
        if (status === "READY_FOR_REVIEW") {
            if (!analysis)
                throw new ProtocolError("analysis", "READY_FOR_REVIEW requires analysis.json");
    ...
    if (pending.action === "APPROVE_MAP") {
        ...
        if (!analysis)
            throw new ProtocolError("analysis", "map approval requires analysis.json");
    ...
    if (pending.action === "APPROVE_PLAN") {
        ...
        if (scope === "STAGES") {
            if (!analysis)
                throw new ProtocolError("analysis", "stage feedback requires analysis.json");
        ...
        if (scope !== "DISCOVERY")
            throw new ProtocolError("event.payload.scope", "must be STAGES or DISCOVERY", scope);
        state.feedback_revision += 1;
        state.analysis_status = "draft";
        state.status = "discovery";
        state.current_stage = null;
    ...
    if (pending.action === "RESOLVE_BLOCKER") {
        if (!state.blocker)
            throw new ProtocolError("state.blocker", "no blocker to resolve");
        const decision = choice(payload, "decision", ["RETRY", "ABORT"]);
        if (decision === "ABORT") {
            state.blocker.retryable = false;
            return { status: "blocked", aborted: true };
        }
        if (!state.blocker.retryable)
            throw new ProtocolError("event.payload.decision", "blocker is not retryable");
...
export async function applyEvent(base, input, eventInput, analysisInput, expectedStateRevision) {
    ...
    const event = record(eventInput, "event");
    exactFields(event, ["transition_id", "type", "payload"], "event");
    const eventDigest = sha(event);
    const applied = state.applied[event.transition_id];
    if (applied) {
        if (applied.event_digest !== eventDigest)
            throw new ProtocolError("event", "transition was already applied with different payload", event.transition_id);
    ...
    if (!state.pending || state.pending.transition_id !== event.transition_id)
        throw new ProtocolError("event.transition_id", "does not match pending transition", event.transition_id);
    const payload = record(event.payload, "event.payload");
    const next = clone(state);
    const pending = next.pending;
    let result;
    if (event.type === "task_failure") {
        if (!AGENT_ACTIONS.has(pending.action))
            throw new ProtocolError("event.type", "task_failure is valid only for agent actions");
        ...
        if (event.type !== expected)
            throw new ProtocolError("event.type", `expected ${expected}`, event.type);
