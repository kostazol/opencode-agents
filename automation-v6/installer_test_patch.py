...
_original_apply = prepare.step6_installer.apply


def apply_with_integrity_aware_baseline(root, log):
    changed = prepare.step6_installer.write_files(
        root,
        {"tests/test_installer_hardening.py": HARDENING_TEST},
    )
    return changed + _original_apply(root, log)


prepare.step6_installer.apply = apply_with_integrity_aware_baseline
