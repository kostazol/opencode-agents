...
Profile selection is a workflow contract. An absent, invalid, or changed profile returns `BLOCKED`. `SINGLE_MODEL` completion uses final validation, fresh cumulative mini review, and post-mini identity confirmation; `FINAL_REVIEW_INPUT_ID` and Terra verdict are `not_applicable`. Manifest records protocol/config version and schema. A compatible update runs recovery before resume. A major mismatch enters `MIGRATION_REQUIRED`; primary requests explicit user consent before migration changes plan state or execution resumes.
